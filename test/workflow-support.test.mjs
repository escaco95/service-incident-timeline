...
test('file format, deterministic layout, 99/100/101 nodes, strict fields and secret-free round trip', async () => {
  ...
  const withoutSecrets = exportWorkflow(base());
  assert.equal(Object.hasOwn(withoutSecrets, 'requiredSecrets'), false);
  assert.deepEqual(importWorkflow(withoutSecrets, { executable: true }).requiredSecrets, []);
  assert.deepEqual(importWorkflow(file(base())), importWorkflow(withoutSecrets));
  for (const requiredSecrets of [null, {}, '', false, 0, ['UNUSED']]) assert.throws(() => importWorkflow({ ...withoutSecrets, requiredSecrets }), /requiredSecrets/);
  ...
  assert.deepEqual(importWorkflow(exported, { executable: true }).requiredSecrets, ['TOKEN']);
  const { requiredSecrets: omitted, ...missingSecrets } = exported;
  assert.throws(() => importWorkflow(missingSecrets), /requiredSecrets/);
  assert.throws(() => importWorkflow({ ...exported, requiredSecrets: ['TOKEN', 'TOKEN'] }), /requiredSecrets/);
...
test('authenticated exchange API validates without mutation, imports OFF, exports no secrets and rejects conflicts', async t => {
  ...
  const exported = (await call(`/api/workflows/${created.id}/export`)).data;
  assert.equal(exported.format, FORMAT);
  assert.equal(Object.hasOwn(exported, 'requiredSecrets'), false);
  const reimported = await call('/api/workflows/validate', 'POST', exported);
  assert.equal(reimported.status, 200); assert.deepEqual(reimported.data.requiredSecrets, []);
