...
2. 서비스 문자열 조건, 외부 URL, 요청값, 응답 배열의 키와 성공·검증 조건을 배포 환경에 맞게 수정한다. 공통값·인증값은 `context` 노드의 키·값 목록에 지정하고 이후 노드에서 `{{context.token}}`처럼 참조한다. `requiredSecrets`는 기존 `secrets` 참조의 호환용이며 새 워크플로우는 빈 배열을 사용한다.
...
화면에서 가져온 구성을 검토한 후 상단 **Dry-Run 테스트**로 이벤트 초기값과 노드별 API 응답을 입력해 흐름을 테스트할 수 있다. 저장 전 편집 내용도 대상이며 실제 API 호출·감사 로그·실행 이력을 생성하지 않는다. 테스트값은 워크플로우별로 별도 저장되어 다음 팝업에서도 복원된다. 워크플로우 교환 JSON에는 포함되지 않는다. [Dry-Run 사용 안내](workflow-transition-plan.md#dry-run-테스트)를 참고한다.
