...
export class Workflows {
  ...
  async save(id, input) {
    await this.vault.mutate(state => {
      ...
      if (flow.enabled) { validateReferences(definition); if (secretNames(definition).some(name => !secrets[name])) throw new AppError(400, '필요한 비밀 변수를 먼저 설정해 주세요.'); }
  ...
  async enable(id, input) {
    await this.vault.mutate(state => {
      ...
      if (input.enabled) { validateDefinition(flow, { executable: true }); validateReferences(flow); if (secretNames(flow).some(name => !flow.secrets[name])) throw new AppError(400, '필요한 비밀 변수를 먼저 설정해 주세요.'); }
  ...
  async run(id, input, { parentId = null } = {}) {
    ...
    const result = await this.vault.mutate(state => {
      ...
      if (secretNames(flow).some(name => !flow.secrets[name])) throw new AppError(400, '필요한 비밀 변수를 먼저 설정해 주세요.');
