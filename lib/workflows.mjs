...
import { diagnoseWorkflow } from './workflow-diagnostics.mjs';
...
export class Workflows {
  ...
  diagnose(id, input) {
    ...
    const availableSecrets = Object.keys(flow.secrets ?? {}).filter(name => flow.secrets[name] && !(plain(input.secrets) && input.secrets[name] === null));
    return diagnoseWorkflow(input, { enabled: flow.enabled, availableSecrets });
  }
  readDryRunSetup(id) {
    this.vault.assertUnlocked();
