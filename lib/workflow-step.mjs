...
export function evaluateStep(node, context, now) {
  ...
  if (TRIGGERS.includes(node.type)) output = context.trigger;
    ...
    const actual = readPath(context, node.config.field), matched = node.config.cases.find(entry => entry.value === actual);
    port = matched?.id ?? 'default'; output = { matched: Boolean(matched), port };
  }
