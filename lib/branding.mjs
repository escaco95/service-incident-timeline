...
const DEFAULT_BRANDING = Object.freeze({ name: 'Service Timeline', subtitle: '서비스 운영 기록', showSubtitle: true, defaultTheme: 'light', timezone: 'UTC' });
...
export function validateBranding(config) {
  ...
  return Object.freeze({ name, subtitle, showSubtitle: config.showSubtitle ?? true, defaultTheme, timezone });
}

function publicBranding(config) {
  // Only these public settings can be returned to an unauthenticated browser.
  return Object.freeze({ name: config.name, subtitle: config.showSubtitle ? config.subtitle : '', defaultTheme: config.defaultTheme, timezone: config.timezone });
}
