...
import { CONFIG_FIELDS, NODE_TYPES, CONTEXT_LIMITS } from '../public/workflow-spec.js';
...
export function validateReferences(definition) {
  ...
  for (const node of nodes) {
    ...
    for (const path of paths) {
      ...
      if (path.startsWith('context.') && !before(node.id).has('$context.' + path.split('.')[1])) fail(`nodes.${node.id}`, `모든 진입 경로에서 먼저 주입되지 않는 컨텍스트 값: ${path}`);
  ...
  return { $schema: 'https://json-schema.org/draft/2020-12/schema', title: 'Workflow exchange v1', type: 'object', additionalProperties: false, required: ['format', 'formatVersion', 'definition', 'requiredSecrets'], properties: {
    ...
    definition: { type: 'object', additionalProperties: false, required: ['name', 'nodes', 'edges'], properties: {
      name: { ...str(80), minLength: 1 }, nodes: { type: 'array', maxItems: LIMITS.nodes, items: { oneOf: NODE_TYPES.map(type => ({ type: 'object', additionalProperties: false, required: ['id', 'type', 'config'], properties: {
        ...
        config: { type: 'object', additionalProperties: false, properties: { ...Object.fromEntries(Object.entries(CONFIG_FIELDS[type]).map(([key, [max, fallback]]) => [key, { ...str(max), default: fallback }])), ...(type === 'context' ? { entries: { type: 'array', maxItems: CONTEXT_LIMITS.entries, default: [], items: { type: 'object', additionalProperties: false, required: ['key', 'value'], properties: { key: { ...str(CONTEXT_LIMITS.key), pattern: '^[a-zA-Z][a-zA-Z0-9_-]{0,63}$', not: { enum: ['constructor', 'prototype', '__proto__'] } }, value: str(CONTEXT_LIMITS.value) } } } } : {}), ...(type === 'http' ? { timeoutMs: { type: 'integer', minimum: 100, maximum: 30000, default: 10000 }, retries: { type: 'integer', minimum: 0, maximum: 3, default: 0 } } : {}) } }
      } })) } },
