...
export function readAudit(state, parameters) {
  ...
  if (filters.kind === 'workflows') {
    rows = (state.workflowRuns ?? []).filter(run => inRange(run.createdAt) && (!filters.workflowId || run.workflowId === filters.workflowId) && (!filters.eventId || run.input.event?.id === filters.eventId) && (!filters.targetId || run.input.event?.services?.some(service => service.id === filters.targetId)) && includes(run.status, filters.result) && includes(run.kind, filters.mode) && includes([run.id, run.workflowName, run.input.event?.title].join(' '), filters.search)).map(run => ({ id: run.id, at: run.createdAt, createdAt: run.createdAt, workflowId: run.workflowId, workflowName: run.workflowName, definitionVersion: run.definitionVersion, eventId: run.input.event?.id, kind: run.kind, status: run.status, startedAt: run.startedAt, finishedAt: run.finishedAt, durationMs: run.durationMs, parentId: run.parentId }));
  } else {
