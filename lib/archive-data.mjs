...
const RUN_STATUSES = ['queued', 'running', 'success', 'failure', 'canceled', 'interrupted'];
export function exportedSettings(state, branding) {
  return { metadata: Object.fromEntries(['schemaVersion', 'revision', 'createdAt', 'updatedAt', 'catalog', 'workflows', 'logPolicy'].filter(key => Object.hasOwn(state, key)).map(key => [key, structuredClone(state[key])])), branding };
}
