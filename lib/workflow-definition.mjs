...
import { NODE_TYPES, TRIGGERS, CONFIG_FIELDS, LIMITS, ports, isChange, validateContextEntries } from '../public/workflow-spec.js';
...
export function validateDefinition(input, { executable = false } = {}) {
  ...
  const nodes = input.nodes.map(node => {
    ...
    if (Object.keys(c).some(key => !Object.hasOwn(fields, key) && !(node.type === 'http' && ['timeoutMs', 'retries'].includes(key)) && !(node.type === 'context' && key === 'entries'))) fail(node.id + ': 지원하지 않는 설정 필드입니다.');
