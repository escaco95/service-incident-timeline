...
import { NODE_TYPES, TRIGGERS, CONFIG_FIELDS, LIMITS, ports, isChange, validateContextEntries, validateSwitchCases } from '../public/workflow-spec.js';
import { readPath, validPath, condition, validateComparison, parseRules, validateDateConfig } from './workflow-values.mjs';
...
const fail = (message, nodeIds = []) => { const error = new AppError(400, message); error.nodeIds = nodeIds; throw error; };
...
export function validateDefinition(input, { executable = false } = {}) {
  ...
  const nodes = input.nodes.map(node => {
    ...
    if (Object.keys(c).some(key => !Object.hasOwn(fields, key) && !(node.type === 'http' && ['timeoutMs', 'retries'].includes(key)) && !(node.type === 'context' && key === 'entries') && !(node.type === 'switch' && key === 'cases'))) fail(node.id + ': 지원하지 않는 설정 필드입니다.');
    if (node.type === 'switch') {
      try { config.cases = validateSwitchCases(c.cases === undefined ? [] : c.cases); } catch (error) { fail(node.id + ': ' + error.message); }
    }
    if (node.type === 'context') {
      try { config.entries = validateContextEntries(c.entries === undefined ? [] : c.entries); } catch (error) { fail(node.id + ': ' + error.message); }
    }
    if (node.type === 'http') {
  ...
  const edges = input.edges.map(edge => {
    ...
    if (!id(edge.id) || edgeIds.has(edge.id) || !from || !to || TRIGGERS.includes(to.type) || from === to) fail('연결 대상과 연결점의 중복을 확인해 주세요.', [...new Set([from?.id, to?.id].filter(Boolean))]);
    if (!ports(from).includes(edge.port)) fail(from.type === 'switch' ? 'switch에 존재하지 않는 분기로 연결되어 있습니다. 분기 목록과 연결을 확인해 주세요.' : '지원하지 않는 출력 연결점입니다.', [from.id]);
    if (outlets.has(`${edge.from}:${edge.port}`)) fail(from.type === 'switch' ? 'switch의 같은 분기에서 여러 노드로 연결할 수 없습니다.' : '같은 출력 연결점에서 여러 노드로 연결할 수 없습니다.', [from.id]);
  ...
  if (executable) {
    const [issue] = executionDiagnostics({ nodes, edges });
...
// Operates on a structurally validated draft; execution and editor diagnostics share these rules.
export function executionDiagnostics({ nodes, edges }) {
    const issues = [];
    const report = (code, message, nodeIds) => issues.push({ code, message, nodeIds });
    const triggers = nodes.filter(node => TRIGGERS.includes(node.type));
    if (triggers.length !== 1) report('trigger-count', '실행하려면 시작 이벤트 노드를 정확히 하나 배치해 주세요.', triggers.map(node => node.id));
    const reachable = new Set(), pending = triggers.map(node => node.id);
    ...
    if (triggers.length && reachable.size !== nodes.length) report('unreachable', '시작 이벤트에서 연결되지 않은 노드가 있습니다.', nodes.filter(node => !reachable.has(node.id)).map(node => node.id));
    for (const node of nodes) {
      ...
      if (node.type === 'switch' && !validPath(c.field)) fail(node.id + ': switch 비교 값의 경로를 확인해 주세요.');
      ...
      if (node.type === 'datetime') validateDateConfig(c, { report: message => report('node-config', node.id + ': ' + message, [node.id]) });
      if (node.type === 'http') {
      ...
      } catch (error) { report('node-config', error.message, [node.id]); }
    }
    return issues;
}
