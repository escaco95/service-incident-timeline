...
import { findItems, dateValue, readPath } from './workflow-values.mjs';
    ...
    try {
      while (node) {
        ...
        try {
          ...
          if (node.type === 'find') { output = findItems(node.config, context); port = output.count === 0 ? 'zero' : output.count === 1 ? 'one' : 'many'; }
          if (node.type === 'datetime') output = dateValue(node.config, context, this.clock());
