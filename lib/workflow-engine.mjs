...
import { activeScope, effectiveServices, reconcileServices, holdService } from './service-state.mjs';
import { readPath } from './workflow-values.mjs';
...
import { cronSlot, TRIGGERS, redact } from './workflow-definition.mjs';
...
export class WorkflowEngine {
  ...
  async unlock() {
    ...
    this.initializing = (async () => {
      ...
      await this.vault.mutate(state => {
        ...
        let changed = false;
        if (state.workflows.some(flow => !flow.deletedAt && flow.nodes.some(node => (node.type === 'service-state' || node.config.executionService)))) changed = reconcileServices(state, at, enqueueRun);
        if (!pending.length && !changed) return NO_CHANGE;
        for (const run of pending) {
          if (run.steps.some(step => step.type === 'http' && (step.status === 'running' || step.attempts?.some(attempt => !attempt.finishedAt)) && isChange(run.definition.nodes.find(node => node.id === step.nodeId)))) holdService(state, run, at);
      ...
      }, { scope: { activeRuns: true, ...activeScope(at) } });
  ...
  candidates(state, from, until) {
    ...
    for (const flow of state.workflows) {
      ...
      if (!root || root.type === 'service-state') continue;
  ...
  async tick() {
    ...
    try {
      ...
      if (this.vault.state.workflows.some(flow => !flow.deletedAt && flow.nodes.some(node => (node.type === 'service-state' || node.config.executionService)))) {
        await this.vault.mutate(state => reconcileServices(state, now, enqueueRun) ? undefined : NO_CHANGE, { scope: { activeRuns: true, ...activeScope(now) } });
      }
      ...
      if (this.closing) return;
      for (const run of (await this.vault.snapshot({ activeRuns: true })).workflowRuns.sort((a, b) => a.createdAt.localeCompare(b.createdAt))) {
        ...
        if (service && (this.vault.state.serviceState?.services.find(item => item.service === service)?.hold && run.definition.nodes.some(isChange) || [...this.jobs.values()].some(job => job.service === service))) continue;
        const controller = new AbortController(), job = { workflowId: run.workflowId, service, controller };
  ...
  async execute(id, controller) {
    ...
    const context = { ...structuredClone(run.input), context: {}, secrets: structuredClone(flow?.secrets ?? {}), nodes: {}, response: null, run: { id, startedAt: run.startedAt, kind: run.kind } };
    ...
    const patch = async change => {
      try { return await this.vault.mutate(state => change(state.workflowRuns.find(item => item.id === id), state), { scope: { ids: { workflowRuns: [id] } } }); }
    ...
    try {
      while (node) {
        ...
        try {
          if (node.type !== 'http') {
            const step = evaluateStep(node, context, this.clock());
            output = step.output; port = step.port; stepStatus = step.status;
            secrets.push(...step.secretValues);
            if (step.stop) { status = step.status; message = step.message; }
          }
          if (node.type === 'http') {
            const c = node.config;
            let failure, uncertain = false;
            for (let attempt = 0; attempt <= c.retries; attempt++) {
              signal.throwIfAborted();
              if (++attempts > LIMITS.attempts) throw new Error('HTTP 시도 예산(100회)을 초과했습니다.');
              const changing = isChange(node);
              try { await this.vault.mutate(state => {
                const live = state.workflowRuns.find(item => item.id === id), service = run.input.trigger.service;
                const latest = state.workflows.find(flow => flow.id === run.workflowId && !flow.deletedAt);
                if (changing && service) {
                  const stored = state.serviceState?.services.find(item => item.service === service);
                  const current = effectiveServices(state.events, this.clock()).get(service)?.severity ?? '';
                  if (stored?.hold) throw Object.assign(new Error('이전 변경 결과 확인이 필요합니다.'), { workflowStatus: 'review' });
                  if (!latest || latest.definitionVersion !== run.definitionVersion || run.kind === 'automatic' && !latest.enabled || live.cancelRequested || current !== run.input.trigger.severity || (stored?.generation ?? 0) !== run.input.trigger.generation) throw Object.assign(new Error('현재 목표 또는 실행 설정이 바뀌어 변경 요청을 생략했습니다.'), { workflowStatus: 'skipped' });
                }
                live.steps.at(-1).attempts.push({ startedAt: this.clock(), number: attempt + 1, intent: changing ? 'change' : 'read' });
              }, { scope: { ids: { workflowRuns: [id] }, ...activeScope(this.clock()) } }); } catch (error) { if (!error.workflowStatus) this.fault = '호출 전 기록 저장에 실패해 후속 실행을 멈췄습니다.'; throw error; }
              try {
                const { url, ...request } = prepareRequest(node, context);
                const response = await this.fetch(new URL(url), { ...request, redirect: 'manual', signal: AbortSignal.any([signal, AbortSignal.timeout(c.timeoutMs)]) });
                let bytes = 0, chunks = [];
                ...
                failure = null; uncertain = false;
              } catch (error) {
                failure = errorMessage(error); uncertain = isChange(node);
              }
              ...
              if (!failure) break;
              signal.throwIfAborted();
              if (uncertain && c.idempotency !== 'verified') break;
              if (attempt < c.retries) await delay(250, undefined, { signal });
            }
            if (failure && uncertain) {
              await patch((item, state) => { holdService(state, item, this.clock()); });
              throw Object.assign(new Error('변경 요청의 적용 여부를 확인해야 합니다.'), { workflowStatus: 'review' });
            }
            if (failure) {
          ...
          if (node.type === 'http' && node.config.outputMode === 'none') storedOutput = { status: output.status, error: output.error };
          if (node.type === 'http' && node.config.outputMode === 'allowlist') storedOutput = Object.fromEntries(node.config.outputPaths.split(',').map(path => path.trim()).filter(Boolean).map(path => [path, readPath(output, path)]));
          if (node.type === 'find') storedOutput = { count: output.count };
          await patch((item, state) => { Object.assign(item.steps.at(-1), { status: stepStatus, finishedAt: this.clock(), port, output: summary(storedOutput, secrets) }); if (status === 'review') holdService(state, item, this.clock()); });
        } catch (error) {
          await patch(item => { Object.assign(item.steps.at(-1), { status: error.workflowStatus ?? (this.closing ? 'interrupted' : controller.signal.aborted ? 'canceled' : 'failure'), finishedAt: this.clock(), error: summary(error.message, secrets) }); });
          throw error;
        }
      ...
      status = error.workflowStatus ?? (this.closing ? 'interrupted' : controller.signal.aborted ? 'canceled' : 'failure');
    ...
    await patch((item, state) => { if (status === 'review' || status !== 'success' && item.steps.some(step => step.attempts?.some(attempt => attempt.intent === 'change' && (!attempt.finishedAt || attempt.status === 'error')))) holdService(state, item, this.clock()); Object.assign(item, { status: item.cancelRequested && status !== 'interrupted' ? 'canceled' : status, message: summary(message, secrets), finishedAt: this.clock(), durationMs: Math.max(0, Date.parse(this.clock()) - Date.parse(item.startedAt)) }); });
  }
