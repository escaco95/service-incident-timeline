...
// Shared only with the server-side archive worker; keys are never sent to clients.
export { encrypt as encryptState, decrypt as decryptState };
...
function readState(state) {
  ...
  return Object.fromEntries(['schemaVersion', 'revision', 'createdAt', 'updatedAt', 'events', 'catalog', 'changes', 'workflows', 'workflowRuns', 'logPolicy', 'logMaintenance', 'resetRequests', 'serviceState'].filter(key => Object.hasOwn(state, key)).map(key => [key, state[key]]));
}
...
export class Vault {
  ...
  async adoptEnvelope(envelope) {
    this.envelope = envelope; this.initialized = true;
    if (!this.key) return;
    const manifest = decrypt(envelope, this.key), daily = new DailyStorage(this.directory);
    let backup;
    try { backup = decrypt(JSON.parse(await fs.readFile(this.file + '.bak', 'utf8')), this.key); } catch {}
    this.state = readState(await daily.load(manifest, (value, scope) => decryptRecords(value, this.key, scope), backup));
    this.daily = daily;
  }
    ...
    try {
      ...
      if (this.initialized) {
        ...
        try {
          ...
          await replaceFile(backupTemp, this.file + '.bak');
